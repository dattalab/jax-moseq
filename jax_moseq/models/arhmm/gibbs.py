...
def resample_precision(seed, x, mask, z, Ab, Q, nu, **kwargs):
    ...
    mahalanobis = jnp.einsum(
        "...i,...ij,...j", residuals, psd_inv(Q)[z], residuals
    )
    a_post = nu[z] / 2 + x.shape[-1] / 2
    b_post = nu[z] / 2 + mahalanobis / 2
    tau = sample_gamma(seed, a_post, b_post)
    return jnp.where(mask[..., get_nlags(Ab) :], jnp.clip(tau, None, 1e2), 1)
...
def resample_nu(
    ...
    **kwargs
    ...
    E_logtau = (masks * jnp.log(tau).reshape(1, -1)).sum(axis=-1) / jnp.clip(
        N, 1, None
    )
    ...
    nu = jax.vmap(_sample_nu, in_axes=(0, 0, 0, 0, 0, 0, None, None))(
        nu, nu_step, thresh, E_tau, E_logtau, N, alpha, beta
    )
    return nu
    ...
    if robust:
        log_likelihoods = jax.lax.map(
            ...
            (
                ...
                kwargs["nu"],
                jnp.tile(
                    mask[na, ..., nlags:], (len(Ab), *(1,) * len(mask.shape))
                ),
            ),
        )
    else:
...
def resample_ar_params(
    seed, *, nlags, num_states, mask, x, z, nu_0, S_0, M_0, K_0, tau, **kwargs
):
    """
...
def resample_model(
    ...
    **kwargs : dict
    ...
    if not states_only:
        ...
        if hypparams["ar_hypparams"]["robust"]:
            states["tau"] = resample_precision(
