...
def init_allocentric_dynamics_params(seed, *, num_states, 
    ...
    sigma_h = jnp.sqrt(inv_gamma_fun(seeds_h, alpha0_h, beta0_h))
    delta_h = jax.vmap(jr.normal)(seeds_h) * sigma_h / jnp.sqrt(lambda0_h)

    seeds_v = jr.split(jr.split(seed)[0], num_states)
    sigma_v = jnp.sqrt(inv_gamma_fun(seeds_v, alpha0_v, beta0_v))
    fun = jax.vmap(partial(jr.normal, shape=(2,)))
    delta_v = fun(seeds_v) * sigma_v[:,na] / jnp.sqrt(lambda0_v)
