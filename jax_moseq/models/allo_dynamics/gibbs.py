...
def resample_heading_dynamics(seed, mask, dh, alpha0_h, beta0_h, lambda0_h):
    ...
    beta_post = beta0_h + 0.5*jnp.sum(mask*(dh - dh_mean)**2) \
        + n * lambda0_h * dh_mean**2 / (2 * (lambda0_h + n))

    sigma_h = jnp.sqrt(sample_inv_gamma(seed, alpha_post, beta_post))
    delta_h = jr.normal(seed) * sigma_h/lambda_post + mu_post
    return delta_h, sigma_h
...
def resample_centroid_dynamics(seed, mask, dv, alpha0_v, beta0_v, lambda0_v):
    ...
    beta_post = beta0_v + 0.5*jnp.sum(mask[...,na]*(dv - dv_mean)**2) \
        + n * lambda0_v * (dv_mean**2).sum() / (2 * (lambda0_v + n))

    sigma_v = jnp.sqrt(sample_inv_gamma(seed, alpha_post, beta_post))
...
def resample_allocentric_dynamics_params(
        ...
        alpha0_v, beta0_v, lambda0_v, num_states, **kwargs):
    """
...
def resample_discrete_stateseqs(seed, h, v, mask, delta_h, sigma_h, 
    ...
    **kwargs : dict
    ...
    log_likelihoods = jax.vmap(allo_log_likelihood, in_axes=(None,None,0,0,0,0))(
        h, v, delta_h, sigma_h, delta_v, sigma_v)
...
def resample_model(data, seed, states, params, hypparams,
    ...
    if not states_only: 
        ...
        params['delta_h'], params['sigma_h'], params['delta_v'], params['sigma_v'] = \
