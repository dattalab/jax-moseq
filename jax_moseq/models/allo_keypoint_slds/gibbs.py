...
from jax_moseq.models.allo_dynamics import (
    compute_delta_heading_centroid, wrap_angle, allo_log_likelihood)
...
def MAP_heading_and_centroid(mask, Y, Y_bar, h, v, obs_variance, 
    ...
    def residual_fun(hv):
        ...
        heading_residuals = ((dh - delta_h[:-1]) * mask[:-1])[:,na] / sigma_h
        centroid_residuals = ((dv - delta_v[:-1]) * mask[:-1,na]) / sigma_v

        # observations
        Y_pred = apply_rotation(Y_bar, h)
        obs_residuals = (Y - Y_pred) / obs_variance[...,na]
        obs_residuals = obs_residuals * mask[...,na]
...
def perturb_heading_and_centroid(seed, mask, Y, Y_bar, h_opt, v_opt, obs_variance,
    ...
    def dynamics_function(hv, t):
        h,v = hv[0], hv[1:]
        R = angle_to_rotation_matrix(h)
        v_next = v + delta_v[t] @ R
        h_next = h + delta_h[t]
        hv_next = jnp.concatenate([h_next, v_next])
        return hv_next * mask[t] + hv * (1-mask[t])

    def emission_function(hv):
        h,v = hv[0], hv[1:]
        return apply_rotation(Y_bar, h) + v
        
    dynamics_covariance = jnp.vstack([sigma_h, sigma_v, sigma_v]).T[:,:,na] * jnp.eye(3)[na,:,:]
    dynamics_covariance = dynamics_covariance * mask[...,na] + 1e-6 * (1-mask[...,na]) * jnp.eye(3)[na,:,:]

    emission_covariance = obs_variance[:,:,na] * jnp.eye(2)[na,:,:]
    emission_covariance = emission_covariance * mask[...,na] + 1e3 * (1-mask[...,na]) * jnp.eye(2)[na,:,:]
        
    params = ParamsNLGSSM(
        initial_mean=jnp.concatenate([h_opt[:1], v_opt[:1]]),
    ...
    hv = extended_kalman_posterior_sample(
        seed, params, Y, jnp.arange(len(mask)))
...
def resample_heading_and_centroid(seed, mask, Y, h, v, x, z, s, Cd, sigmasq, 
    ...
    **kwargs : dict
    ...
    h,v = jax.vmap(perturb_heading_and_centroid)(
        seeds, mask, Y[...,:2], Y_bar, h_opt, v_opt, obs_variance, 
        delta_h[z], sigma_h[z], delta_v[z], sigma_v[z])
