...
def init_model(hypparams=None, allo_hypparams=None, seed=jr.PRNGKey(0), **kwargs):
    """
    ...
    model = keypoint_slds.init_model(
        hypparams=hypparams, seed=seed, **kwargs)
    
    if allo_hypparams is not None:
        model['hypparams']['allo_hypparams'] = allo_hypparams

    (model['params']['delta_h'], model['params']['sigmasq_h'],
     model['params']['delta_v'], model['params']['sigmasq_v']
    ) = init_allocentric_dynamics_params(seed, **model['hypparams']['allo_hypparams'])
    
    return model

def _check_init_args(hypparams, allo_hypparams):
    assert (hypparams is not None and 'allo_hypparams' in hypparams) or (allo_hypparams is not None), \
